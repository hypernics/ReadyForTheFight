...
function ReadyForTheFight:HaveGlyph(glyph) 
	...
	for i = 1, NUM_GLYPH_SLOTS do
		...
		if ( enabled ) then 
			for j = 1, GetNumGlyphs() do
				if (select(5, GetGlyphInfo( j )) == glyphID) then
					return true;
				end
			end
		end
...
frame:RegisterEvent("ADDON_LOADED");
frame:RegisterEvent("ACTIVE_TALENT_GROUP_CHANGED");
>>>>>>> HaveGlyph és HaveTalent globális fügvények lettek, HaveGlyph javítás:ReadyForTheFight/ReadyForTheFight.lua
